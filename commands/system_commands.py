...
def register_system_commands(command_system, falar, ouvir_comando, audio_processor=None):
    ...
    def dizer_ola(falar_func=None, ouvir_func=None):
        (falar_func or falar)("Olá! Eu sou o R2, seu assistente pessoal. Como posso ajudar?")

    def dizer_hora(falar_func=None, ouvir_func=None):
        hora_atual = datetime.datetime.now().strftime("%H:%M")
        (falar_func or falar)(f"Agora são {hora_atual}.")

    def dizer_data(falar_func=None, ouvir_func=None):
        data_atual = datetime.datetime.now().strftime("%d de %B de %Y")
        (falar_func or falar)(f"Hoje é {data_atual}.")
    ...
    def abrir_programa(nome_programa, falar_func=None, ouvir_func=None):
        ...
        try:
            ...
            (falar_func or falar)(f"Abrindo {nome_programa}.")
        except Exception as e:
            logger.error(f"Erro ao abrir programa: {e}")
            (falar_func or falar)(f"Não foi possível abrir o programa {nome_programa}.")
    ...
    def tirar_print(falar_func=None, ouvir_func=None):
        try:
            ...
            (falar_func or falar)(f"Print da tela salvo como {filename}.")
        except Exception as e:
            logger.error(f"Erro ao tirar print: {e}")
            (falar_func or falar)("Não foi possível tirar o print da tela.")

    def preencher_documento(falar_func=None, ouvir_func=None):
        (falar_func or falar)("O que você gostaria que eu escrevesse?")
        texto_a_escrever = (ouvir_func or ouvir_comando)()

        if texto_a_escrever:
            (falar_func or falar)(f"Preparando para escrever: {texto_a_escrever}.")
            pyperclip.copy(texto_a_escrever)
            (falar_func or falar)("Por favor, clique onde você deseja que o texto seja inserido. Aguardando 5 segundos...")
            time.sleep(5)

            try:
                pyautogui.hotkey('ctrl', 'v')
                (falar_func or falar)("Texto inserido com sucesso.")
            except Exception as e:
                logger.error(f"Erro ao colar texto: {e}")
                (falar_func or falar)("Não foi possível colar o texto. Verifique se a janela do documento está ativa.")
        else:
            (falar_func or falar)("Não entendi o que você disse. Por favor, tente novamente.")
    ...
    def mutar_audio(falar_func=None, ouvir_func=None):
        try:
            ...
            (falar_func or falar)("Áudio mutado.")
        except Exception as e:
            logger.error(f"Erro ao mutar áudio: {e}")
            (falar_func or falar)("Não foi possível mutar o áudio.")
    ...
    def desmutar_audio(falar_func=None, ouvir_func=None):
        try:
            ...
            if platform.system() == "Windows":
                import ctypes
                ctypes.windll.user32.keybd_event(0xAD, 0, 0, 0)  # VK_VOLUME_MUTE
            (falar_func or falar)("Áudio desmutado.")
        except Exception as e:
            logger.error(f"Erro ao desmutar áudio: {e}")
            (falar_func or falar)("Não foi possível desmutar o áudio.")
    ...
    def aumentar_volume(falar_func=None, ouvir_func=None):
        try:
            ...
            (falar_func or falar)("Volume aumentado.")
        except Exception as e:
            logger.error(f"Erro ao aumentar volume: {e}")
            (falar_func or falar)("Não foi possível aumentar o volume.")
    ...
    def diminuir_volume(falar_func=None, ouvir_func=None):
        try:
            ...
            (falar_func or falar)("Volume diminuído.")
        except Exception as e:
            logger.error(f"Erro ao diminuir volume: {e}")
            (falar_func or falar)("Não foi possível diminuir o volume.")

    def configurar_voz(falar_func=None, ouvir_func=None):
        """Configura as preferências de voz."""
        if not audio_processor:
            (falar_func or falar)("Sistema de configuração de voz não disponível.")
            return

        (falar_func or falar)("Deseja alterar velocidade, volume ou tom da voz?")
        resposta = (ouvir_func or ouvir_comando)()
        ...
        if 'velocidade' in resposta or 'rápido' in resposta or 'devagar' in resposta:
            (falar_func or falar)("Diga 'mais rápido', 'mais devagar' ou 'velocidade normal'")
            comando_vel = (ouvir_func or ouvir_comando)()
            
            if comando_vel:
                if 'mais rápido' in comando_vel or 'acelerar' in comando_vel:
                    audio_processor.set_voice_rate(200)  # Aumenta a velocidade
                    (falar_func or falar)("Velocidade da voz aumentada.")
                    ...
                    (falar_func or falar)("Velocidade da voz reduzida.")
                else:
                    audio_processor.set_voice_rate(150)  # Velocidade normal
                    (falar_func or falar)("Velocidade da voz definida como normal.")
                
        elif 'volume' in resposta:
            (falar_func or falar)("Diga 'aumentar volume', 'diminuir volume' ou 'volume normal'")
            comando_vol = (ouvir_func or ouvir_comando)()
            
            if comando_vol:
                if 'aumentar' in comando_vol:
                    audio_processor.set_voice_volume(1.0)  # Volume máximo
                    (falar_func or falar)("Volume da voz aumentado.")
                elif 'diminuir' in comando_vol:
                    audio_processor.set_voice_volume(0.5)  # Volume médio
                    (falar_func or falar)("Volume da voz diminuído.")
                else:
                    audio_processor.set_voice_volume(0.8)  # Volume normal
                    (falar_func or falar)("Volume da voz definido como normal.")
            
        elif 'tom' in resposta or 'voz' in resposta:
            (falar_func or falar)("Diga 'tom mais alto', 'tom mais baixo' ou 'tom normal'")
            comando_tom = (ouvir_func or ouvir_comando)()
            
            if comando_tom:
                if 'mais alto' in comando_tom or 'alto' in comando_tom:
                    audio_processor.set_voice_pitch(150)  # Tom mais alto
                    (falar_func or falar)("Tom da voz aumentado.")
                elif 'mais baixo' in comando_tom or 'baixo' in comando_tom:
                    audio_processor.set_voice_pitch(80)   # Tom mais baixo
                    (falar_func or falar)("Tom da voz diminuído.")
                else:
                    audio_processor.set_voice_pitch(110)  # Tom normal
                    (falar_func or falar)("Tom da voz definido como normal.")
            
        else:
            (falar_func or falar)("Opção não reconhecida. Use velocidade, volume ou tom.")

    def listar_vozes(falar_func=None, ouvir_func=None):
        """Lista vozes disponíveis (apenas para TTS offline)."""
        if not audio_processor:
            (falar_func or falar)("Sistema de listagem de vozes não disponível.")
            return

        voices = audio_processor.list_available_voices()
        if not voices:
            (falar_func or falar)("Nenhuma voz disponível para listar.")
            return

        (falar_func or falar)(f"Encontradas {len(voices)} vozes disponíveis:")
        for i, voice in enumerate(voices[:5]):  # Mostra apenas as primeiras 5
            (falar_func or falar)(f"{i+1}. {voice['name']}")

    def alterar_voz(falar_func=None, ouvir_func=None):
        """Altera para uma voz específica."""
        if not audio_processor:
            (falar_func or falar)("Sistema de alteração de voz não disponível.")
            return

        voices = audio_processor.list_available_voices()
        if not voices:
            (falar_func or falar)("Nenhuma voz disponível para alterar.")
            return

        (falar_func or falar)(f"Vozes disponíveis. Diga o número da voz desejada (1 a {len(voices)})")
            ...
            if resposta:
                ...
                if 1 <= numero <= len(voices):
                    if audio_processor.change_voice(numero - 1):
                        (falar_func or falar)(f"Voz alterada para: {voices[numero-1]['name']}")
                    else:
                        (falar_func or falar)("Erro ao alterar a voz.")
                else:
                    (falar_func or falar)(f"Número inválido. Use um número entre 1 e {len(voices)}")
        except ValueError:
            (falar_func or falar)("Não entendi o número. Por favor, tente novamente.")

    def modo_voz_offline(falar_func=None, ouvir_func=None):
        """Ativa o modo de voz offline."""
        if not audio_processor:
            (falar_func or falar)("Sistema de voz offline não disponível.")
            return

        # Esta funcionalidade precisaria ser expandida para realmente alterar o modo
        (falar_func or falar)("Modo de voz offline ativado. Usando síntese de voz local.")
    ...
    def modo_voz_online(falar_func=None, ouvir_func=None):
        ...
        if not audio_processor:
            (falar_func or falar)("Sistema de voz online não disponível.")
            return

        (falar_func or falar)("Modo de voz online ativado. Usando serviço de síntese de voz em nuvem.")
    ...
    def testar_voz(falar_func=None, ouvir_func=None):
        ...
        (falar_func or falar)(frase_teste)
    ...
    command_system.register_command("vozes disponíveis", listar_vozes, "Lista vozes disponíveis")
