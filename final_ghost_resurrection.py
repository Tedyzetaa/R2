# filename: final_ghost_resurrection.py
import os

FILE_PATH = "main2.py"

def restore_ghost_protocol():
    print("💎 [RESTAURAÇÃO]: Reativando a estética original GHOST PROTOCOL...")
    ...
    # A Interface Original de Alta Fidelidade com JS Blindado
    # Usamos r""" para que o Python ignore as barras invertidas do JavaScript
    ghost_ui = r'''
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="pt-br">
...
'''
    ...
    # Localização e substituição agressiva do bloco HTML no main2.py
    try:
        # Encontra o bloco atual do HTML_TEMPLATE (pode estar com " ou ')
        import re
        pattern = re.compile(r'HTML_TEMPLATE\s*=\s*""".*?"""', re.DOTALL)
        
        if not pattern.search(content):
            # Tenta com aspas simples caso o patch nuclear tenha mudado
            pattern = re.compile(r"HTML_TEMPLATE\s*=\s*'''.*?'''", re.DOTALL)

        if not pattern.search(content):
            print("❌ Falha crítica: Bloco HTML_TEMPLATE não localizado no código.")
            return

        final_content = pattern.sub(ghost_ui.strip(), content)

        with open(FILE_PATH, "w", encoding="utf-8") as f:
            f.write(final_content)
        
        print("✅ [MISSÃO CUMPRIDA]: O visual original GHOST PROTOCOL foi restaurado!")
        print("🚀 JavaScript blindado e menus táticos reativados.")
        print("🔄 Por favor, feche e abra o R2 para validar as alterações.")

    except Exception as e:
        print(f"❌ Erro na restauração: {e}")

if __name__ == "__main__":
    restore_ghost_protocol()