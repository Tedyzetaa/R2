# filename: resurrection_ghost_ui.py
import os

FILE_PATH = "main2.py"

def apply_resurrection():
    print("💎 [RESTAURAÇÃO]: Reativando estética Ghost Protocol e corrigindo Córtex JS...")
    ...
    # O Template Original com as correções de JS injetadas com segurança
    new_template = r'''
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="pt-br">
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body::before {
      ...
      background-size: 100% 4px, 3px 100%; z-index: 9999; opacity: 0.15;
    }
    ...
    #status-led { width: 10px; height: 10px; border-radius: 50%; transition: 0.3s; box-shadow: 0 0 10px currentColor; }
    ...
    .rm-btn { cursor: pointer; color: var(--text-muted); padding: 0 4px; }
    ...
    body.drag-over::after {
      content: 'TRANSMISSÃO DE DADOS DETECTADA: SOLTE ARQUIVOS'; position: fixed; inset: 0;
...
<body>
  ...
  <main>
    <header>
      <div style="font-weight:700; letter-spacing:2px; font-family: 'Share Tech Mono';">GHOST_PROTOCOL_V5.0</div>
  ...
  <script>
    ...
    function sendMsg() {
      ...
      const rawMsg = box.value;
      const msg = rawMsg.replace(/^\s+|\s+$/g, '');
...
'''
    ...
    # Localização segura do bloco HTML no main2.py
    try:
        ...
        # Busca o fechamento das aspas triplas após o marcador de início
        ...
        # Ajusta para incluir as aspas de fechamento
        ...
        print("✅ [SUCESSO]: O visual 'Ghost Protocol' foi restaurado!")
        print("🚀 JavaScript blindado contra erros de escape.")
        print("🔄 Reinicie o main2.py e desfrute da interface original.")

    except Exception as e:
        print(f"❌ Falha crítica na restauração: {e}")
